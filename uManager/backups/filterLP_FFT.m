...
for i=1:numImgs
    ...
    img_filtd_1 = abs(ifft2(T1.*fft2(temp_img)));
    img_filtd_1 = img_filtd_1 - min(min(img_filtd_1));
    img_filtd_1 = img_filtd_1./max(max(img_filtd_1));
    img_filtd_2 = abs(ifft2(T2.*(fft2(temp_img))));
    img_filtd_2 = img_filtd_2 - min(min(img_filtd_2));
    img_filtd_2 = img_filtd_2./max(max(img_filtd_2));
    img_filtd_3 = abs(ifft2(T3.*(fft2(temp_img))));
    img_filtd_3 = img_filtd_3 - min(min(img_filtd_3));
    img_filtd_3 = img_filtd_3./max(max(img_filtd_3));
    
    figure, imagesc((uint16(img_filtd_1))), colormap gray
    title('filtered image - LP')
    figure, imagesc((uint16(img_filtd_2))), colormap gray
    title('filtered image, HP - scaled')
    figure, imagesc((uint16(img_filtd_3))), colormap gray
...
%     img_filtd_1 = img_filtd_1-min(min(img_filtd_1));
%     img_filtd_1 = img_filtd_1/max(max(img_filtd_1));
%     figure, imshow(img_filtd_1), title('manually scaled')
    
    % lets check out the gradient of this mofo
    maskX = [-1 0 -1; -2 0 2; -1 0 1];
    maskY = [-1 -2 -1; 0 0 0; 1 2 1];
    derX = conv2(double(img_filtd_1), maskX);
    derY = conv2(double(img_filtd_1), maskY);
    gradMag = sqrt(derX.^2 + derY.^2);
    gradMag = gradMag/max(max(gradMag));
    
    [~,thr1] = edge(img_filtd_1,'sobel');
    bwIMG1 = edge(img_filtd_1, 'sobel', thr1*.6);
    [~,thr2] = edge(img_filtd_2,'sobel');
    bwIMG2 = edge(img_filtd_2, 'sobel', thr2*.9);
    [~,thr3] = edge(img_filtd_3,'sobel');
    bwIMG3 = edge(img_filtd_3, 'sobel', thr3*.5);

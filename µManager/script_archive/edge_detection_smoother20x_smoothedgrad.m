...
%     thresh = modl.mu+modl.sigma/2;
    ...
    % try to find circles
    [centers, radii, metric] = imfindcircles(gradMag_sm, [35 55]);
    
    % try thresholding smoother gradient for edge detection
    gradThresh = graythresh(gradMag_sm);
    bwGrad = im2bw(gradMag_sm, gradThresh);
...
%     figure, imshow(bwC), title('bwC');
    ...
    figure, imshow(bwGrad), title('thresholded smoothed gradient image')

...
% FFT, rather than the log polar transform of the original image!
  ...
  function Register()
    ...
    I1 = imread('Cell1_BFP.tif');
    I1 = I1-min(min(I1));
    I1 = uint16(I1/max(max(I1)));
    
    dims = size(I1);
    pattern_temp = floures_pattern_gen(43, 70, dims, 4);
    imwrite(uint16(pattern_temp * (2^16-1)), 'pattern.tif')
    
    

    % Load second image (I2)

    I2 = imread('pattern.tif');
    
    sizediff = size(I1,2)-size(I2,2);
    I2 = [I2, zeros(size(I2,1), sizediff)];

...
img_sob_stack = zeros( size( imgStack ) );
...
for i = 1:imgCount
    ...
    img_sob_stack(:,:,i) = imgradient(im);
end

add_time_base =  toc;

img_sob = img_stack_to_img_2D( img_sob_stack, [15 15] );
...
% load( 'sobropy_rand_samp_data.mat', 'summary_stats' )
stat_idx = 1;
...
for i = 5:10:size( imgStack, 3 )
    ...
    train_sob = zeros( 600, 600, length( rand_idx ) );
    
    for j = 1:length( rand_idx )
        train_sob(:,:,j) = img_sob_stack(:,:,rand_idx(j) );
    end
    
    train_sob = imclose( train_sob, se );
    train_sob_vector = train_sob(:);
    ...
    gmm = fitgmdist(train_sob_vector(1:skip_size:end), num_clusts, 'replicates',3, 'Options', options);
...
save( 'sobel_rand_samp_data.mat', 'summary_stats' )

function xout = remove_spurious_transit(xin,minSize,varargin)
%written by
%C.P.Richter
...
while iter
    dX = diff(xout);
    if sum(dX~=0) <= 1 %trace shows no intensity change
        break
    end %if
    idxNZ = find(dX); %position of intensity change
    
%     ddX = sign(dX(idxNZ(1:end-1))) + sign(dX(idxNZ(2:end))) + diff(sign(dX(idxNZ)));
%     ddX(diff(abs(dX(idxNZ))) == 0) = 0; %
%     ddX = diff(sign(dX(idxNZ)));
    
    ddX = sign(dX(idxNZ(1:end-1))) + sign(dX(idxNZ(2:end)));
